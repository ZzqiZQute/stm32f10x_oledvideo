...
MP3DecInfo *AllocateBuffers(void)
{
  ...
  ClearBuffer((void*)&mp3DecInfo, sizeof(MP3DecInfo));
   ...
   // return mp3DecInfo_pointer;
	...
	SubbandInfo *sbi;
#ifdef USE_mymalloc
	mp3DecInfo_pointer = (MP3DecInfo *)mymalloc(sizeof(MP3DecInfo));
#else
	...
	mi =  (IMDCTInfo *)       mymalloc(sizeof(IMDCTInfo));
	hi =  (HuffmanInfo *)     mymalloc(sizeof(HuffmanInfo));	
	di =  (DequantInfo *)     mymalloc(sizeof(DequantInfo));
	fh =  (FrameHeader *)     mymalloc(sizeof(FrameHeader));
	si =  (SideInfo *)        mymalloc(sizeof(SideInfo));
	sfi = (ScaleFactorInfo *) mymalloc(sizeof(ScaleFactorInfo));
...
void FreeBuffers(MP3DecInfo *mp3DecInfo)
{
    ...
    if (!mp3DecInfo)
		return;
#ifdef USE_mymalloc
	myfree(mp3DecInfo->FrameHeaderPS);
	myfree(mp3DecInfo->SideInfoPS);
	myfree(mp3DecInfo->ScaleFactorInfoPS);
	myfree(mp3DecInfo->HuffmanInfoPS);
	myfree(mp3DecInfo->DequantInfoPS);
	myfree(mp3DecInfo->IMDCTInfoPS);
	myfree(mp3DecInfo->SubbandInfoPS);
